...
So we can use the '_' char to "ignore" elements on the list. The underscore is a wildcard variable. Here, I'm using the pipe operator to differentiate between the head and tail of the list.
...
The last element (after the cons operator) has to be a list to build a proper list. If you don't do so you will create what is knows as an unproper list. On an unproper list the tail will be treated as an atom and not a list. You can see the example below, the first time we build an unproper list and we get the tail several times (this is very common in Erlang when processing a list using recursion, here I'm doing it manually just to show the problem).
