...
This scenario is the one that can be problematic if you want to model your permissions at the end point level.

h3. Customers end points.

You will have the following end-points for the Customer object.
...
The implementation of the DELETE operation can be a logical delete while preserving the data in your data store.

In this scenario any update to customers is done via PUT, but while we want to grant access to update to multiple roles. We only want a subset of those roles to be able to change the status of the customer.

So we can't really use this end point to change status without changing where we implement security.
