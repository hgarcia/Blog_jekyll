...
{% highlight cs%}
	public void CreateClassFromUsage() 
	{
		var obj = new MyNewCLass("someVariable"); 
		obj.GetSumOfTwoNumbers(3, 5);
		var originalString = "This is the title."; 
		var stringsToAdd = new[]{"First line","Second line"}; 
		string concatenatedString = obj.ConcatTwoStrings(originalString, stringsToAdd); 
	}
{% endhighlight %}
...
{% highlight cs%}        
	...
	namespace dynamicDemo 
	{
		publicclass MyNewCLass 
		{
			...
			public int GetSumOfTwoNumbers(int p, int p_2) 
			{
				return p+p_2; 
			}
			
			public string ConcatTwoStrings(string originalString, string[] stringsToAdd) 
			{
...
Notice the name of the parameters, in those ones where I used variables the parameters
have proper names, the other parameters names are kind of ugly and I hope they fix
that (p, p_2?). My suggestion will be to use the type plus some contextual information
or at least the type plus the word parameter and them a number if you need it. In
the case of strings the whole string or the first word if it's a phrase. All in all
it's a nice feature to come with the IDE.