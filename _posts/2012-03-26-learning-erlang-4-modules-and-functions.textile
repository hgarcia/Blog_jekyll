---
layout: post
title: Modules and functions, Erlang building blocks.
...
We name the module using the "module" directive, this has to be the first directive on a module. The name of the module have to be the same as the name of the file without the extension (erl) and we use an atom not a string.

A Module provide encapsulation, by default all functions defined inside a module are private to it; to expose functions outside the module you need to export them using the export directive.

The export directive takes a list containing the function name and the arity. (name/arity)
...
There are more ways to compile modules and a myriad of directives that can be used both by the compiler and in code. I will cover them up in a future post.
