...
{% highlight aspx-cs %}
	...
	{
		var page = new HtmlPage("../SiteToTest/HTMLPage.htm");
		page.Elements.Find("openWindow").Click(WaitFor.None);
		
		var handler = new BrowserCommandHandler
		{
			PopupAction = PopupAction.None,
			RequiresElementFound = true,
			ClientFunctionName = BrowserCommand.FunctionNames.ClickElement
		};
		
		var target = new BrowserCommandTarget
		{
			WindowIndex = 1,
			Id = "clickedOnTheSecondWindow"
		};
		
		handler.SetArguments(false);
		
		var command = new BrowserCommand(BrowserCommand.FunctionNames.ClickElement)
		{
			Description = "Click",
			Target = target,
			Handler = handler
		...
		var info = page.ExecuteCommand(command);
	}
{% endhighlight %}
