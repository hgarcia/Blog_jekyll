...
summary:    I don’t remember if I read this on Clean Code or Refactoring or maybe I just hear it somewhere, but It’s so true.    It’s not that you can’t change the code and that you can’t apply refactoring techniques. The problem is that each ch...
---
  ...
  First I did a few changes and I get the program working in my machine. There were some strings hardcoded that reference paths to some external files.   <br />I also create two projects, one for the unit tests and one for a library (the project is a console app at this moment).
