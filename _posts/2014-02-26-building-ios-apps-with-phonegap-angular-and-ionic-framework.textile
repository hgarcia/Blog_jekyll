...
* "Part 2":http://blog.dynamicprogrammer.com/2014/02/28/building-the-basic-UI-and-some-geolocation-work-part-2.html
...
and install the client side dependencies.
...
<pre><code>
    ...
    node install karma-phantomjs-launcher -D
    node install karma-jasmine -D
    node install karma-ng-scenario -D
    node install karma-junit-reporter -D
    node install karma -D
    node install karma-coverage -D
    node install phantomjs -D
...
You should signup into "Travis":http://travis-ci.com and follow the instructions on http://docs.travis-ci.com/user/getting-started/ to setup your project.
...
We will start working in the application with the File and Geo-location API's to track our activity and save that activity on files to upload to a server.