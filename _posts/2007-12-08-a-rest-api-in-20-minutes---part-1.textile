...
summary: If you haven't look at it, take a look at the Subsonic project, is really cool and can generate a ORM infrastructure faster than any other framework I tried out there.
...
It took this idea from the <a target="_blank" href="http://subsonicproject.com/view/rest-linq-and-you.aspx">HttpHandler implementation on Subsonic</a>
...
using System;
	...
	namespace LaTrompa.Services
	{
		...
		{
			object GetRequestedObject(UriProcessor up);
		}
	}
...
{% hightlight cs %}
	...
	namespace LaTrompa.Services
	{
		public class UriProcessor
		{
			...
			public UriProcessor(Uri url)
			{
				if (url.Segments.Length != 5)
				{
					throw new UriMalformedException();
				}
				this._web = url.Segments[1].Replace("/", "");
				this._objectType = url.Segments[2].Replace("/", "");
				this._objectName = url.Segments[3].Replace("/", "");
				
				string[] com = url.Segments[4].Split('.');
				if (com.Length != 2)
				{
					throw new UriMalformedException();
				}
				this._command = com[0];
				this._returnFormat = com[1];
				this._protocol = url.Scheme;
				this._domain = url.Host;
				this._qs = HttpUtility.ParseQueryString(url.Query);
			}
		}
	}
{% endhighlight %}
...
{% hightlight cs %}
	...
	namespace LaTrompa.Services
	{
		public class UriMalformedException : Exception
		{
			public UriMalformedException() : base("The uri has the wrong format")
			{
			}
		}
	}
	...
	namespace LaTrompa.Services
	{
		public class ServiceHandler : IHttpHandler
		{
			...
			public void ProcessRequest(HttpContext context)
			{
				...
				IServiceable service = ServiceHandlerFactory.Create(up);
